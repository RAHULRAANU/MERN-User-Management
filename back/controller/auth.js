...
exports.register = async (req, res, next) => {
  ...
  const { name, email, phone, gender, howDidYouHear, city, state, password, password2 } = req.body;

  if (!name || !email || !password || !password2 || !phone || !city || !state || !howDidYouHear || !gender) {
    return next(res.status(404).json({ "Error": "Please Enter All Fields" }));
  }

  if (password !== password2) {
    return next(res.status(404).json({ "Error": "Passwords does not match" }));
  }
